...
//
// DATABASE_URL empty  => DB disabled, reserve routes fall back to the in-memory
//                        map (so local/unit runs need no Postgres).
// DATABASE_URL set     => real pgxpool; migrations applied on startup; the reserve
...
func initDB(ctx context.Context) error {
	if !dbEnabled() {
		log.Printf("inventory-service: DATABASE_URL not set — DB disabled, using in-memory store")
		return nil
	}
...
func runMigrations(ctx context.Context) error {
	...
	for _, f := range files {
		...
		log.Printf("inventory-service: applied migration %s", f)
	}
	return nil
}

// dbCreateReservation does a real INSERT then SELECT it back against the
// inventory table.
func dbCreateReservation(ctx context.Context, sku string, qty int) (*Reservation, error) {
	id := fmt.Sprintf("inv_%d", time.Now().UnixNano())
	if _, err := pool.Exec(ctx,
		"INSERT INTO inventory (id, sku, qty) VALUES ($1,$2,$3)", id, sku, qty); err != nil {
		return nil, err
	}
	res := &Reservation{}
	if err := pool.QueryRow(ctx,
		"SELECT id, sku, qty FROM inventory WHERE id=$1", id).
		Scan(&res.ID, &res.SKU, &res.Qty); err != nil {
		return nil, err
	}
	return res, nil
}

// dbGetReservation returns nil (no error) when the reservation does not exist.
func dbGetReservation(ctx context.Context, id string) (*Reservation, error) {
	res := &Reservation{}
	err := pool.QueryRow(ctx,
		"SELECT id, sku, qty FROM inventory WHERE id=$1", id).
		Scan(&res.ID, &res.SKU, &res.Qty)
